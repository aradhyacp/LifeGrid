...
function populateDevices() {
  ...
  const defaultDevice =
    devices.find((d) => d.name.includes("16 Pro")) || devices[0];
...
function updatePreview() {
  if (!state.selectedType) {
    elements.previewScreen.innerHTML =
      '<div class="preview-placeholder"><span>Select a wallpaper type</span></div>';
    return;
  }
...
function drawYearPreview(ctx, width, height) {
  ...
  const padding = width * 0.2; // 20% horizontal padding
...
function drawGoalPreview(ctx, width, height) {
  ...
  if (state.goalDate) {
    ...
    daysRemaining = Math.max(
      0,
      Math.ceil((goal - now) / (1000 * 60 * 60 * 24)),
    );
